...
export default class LoadingAddon extends React.PureComponent {
  ...
  getLoadingAddon() {
    ...
    switch (type) {
      case 'pendule': // 摆钟
        return (
          <div className={classnames('nui-loading-addon')}>
            <div />
            <div />
          </div>
        )
      case 'cyclone': // 旋风
        return <div className={classnames('nui-loading-addon')} />
      default:
        // 喷泉、波纹
        return (
          <div className={classnames('nui-loading-addon')}>
  ...
  render() {
    ...
    const typeClass = `nui-loading-${type}`
    ...
    return (
      <div {...others} className={classnames('nui-loading', typeClass, className)}>
        {loadingAddon}
        <div className='nui-loading-message'>
          {loadingMsg}
          <span className='nui-loading-message-dotted'>{dotted}</span>
        </div>
      </div>
    )
  }
